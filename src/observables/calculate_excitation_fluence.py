...
calculated with :class:`qdmnp.full_qs_model.FullQSSpheroidPulseModel`.  An
...
def _resolved_settings(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for key in (
        ...
        if key == "points_per_fastest_cycle":
            minimum = 8
        if key == "workers":
            minimum = 0  # 0 = all logical CPUs except two (parallel.resolve_workers)
