...
* :class:`qdmnp.rational_fit.HybridQDPlasmonModel` (central point-dipole/DD),
* :class:`qdmnp.full_qs_model.FullQSSpheroidPulseModel` (full local-QS/FQS),
* :class:`qdmnp.spheroid_green.LegacyDipoleInteraction` and the analytic
...
from qdmnp.observables.spectral_features import (
...
from qdmnp.observables.threshold_metrics import (
    resolved_threshold_mask,
    threshold_from_curve,
)
from qdmnp.observables.parallel import process_pool, resolve_workers, shared_fit_cache
from qdmnp.spheroid_pulse_comparison import _legacy_tail_ratio
...
def _source_hashes(extra_paths: Iterable[Path] = ()) -> dict[str, str]:
    paths = [
        ...
        PACKAGE_ROOT / "observables" / "spectral_features.py",
        PACKAGE_ROOT / "observables" / "threshold_metrics.py",
        PACKAGE_ROOT / "observables" / "dd_pulse.py",
        PACKAGE_ROOT / "spheroid_pulse_comparison.py",
        PACKAGE_ROOT / "rational_fit.py",
        PACKAGE_ROOT / "passive_fit.py",
        PACKAGE_ROOT / "full_qs_model.py",
        PACKAGE_ROOT / "spheroid_green.py",
        PACKAGE_ROOT / "spheroid_equatorial.py",
        *extra_paths,
    ]
    hashes: dict[str, str] = {}
    for path in paths:
...
def _base_metadata(
    ...
    *,
    ...
    return {
        ...
        "model_api": {
            "dd_time": "qdmnp.rational_fit.HybridQDPlasmonModel.solve",
            "dd_frequency": "qdmnp.spheroid_green.LegacyDipoleInteraction.frequency_response",
            "fqs_time": "qdmnp.full_qs_model.FullQSSpheroidPulseModel.solve",
            "fqs_frequency": "analytic spheroidal Green kernel response_from_material",
            "linear_coupling": "qdmnp.spheroid_green.solve_linear_hybrid_response",
        },
